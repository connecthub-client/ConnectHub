...
`VpnProfile.avoid_default_route` (column default `1`, backfilled via `add_column_if_missing` the same way as `hosts.vpn_profile_id`) exists so multiple profiles can be connected simultaneously without one taking over the default route out from under another - `vpn::effective_config` appends `pull-filter ignore "redirect-gateway"` to the config actually written to disk when it's set, filtering out that specific pushed option while leaving whatever subnet-specific routes the server pushes intact. This was added after real multi-VPN testing showed the failure mode directly: with two profiles up at once, both trying to claim 0.0.0.0/0, the second would fail to connect properly and the host machine's own internet connectivity could break too.

VPN is entirely automatic from the user's perspective - there's no manual VPN button anywhere. The gating lives in **one place**, `AppShell.tsx::ensureVpnUp(host)`: if the host has a `vpn_profile_id` and it isn't already connected, it connects the VPN first and only returns true once that succeeds (surfacing an error and returning false otherwise), rather than letting an SSH attempt silently time out against an unreachable private IP. `handleConnect`/`handleOpenSftp` both call it before opening a session. This must stay centralized in `AppShell` rather than duplicated per-caller - an earlier version put the check only inside `HostContextPanel`'s button handlers, which missed `HostTree.tsx`'s double-click and right-click "Connect" menu entirely (both call `onConnectHost` → `AppShell`'s handler directly), so connecting that way skipped the VPN and went straight to a doomed SSH attempt. `HostContextPanel` now just receives `vpnBusy`/`vpnError` as props (scoped to the currently-displayed host by id, so an error connecting one host can't linger and appear against a different one you navigate to afterward) for its own busy/error display, but the actual decision is made in `AppShell`. `TunnelForm.tsx` does its own equivalent check for tunnels started directly from the Tunnels tab (not just via a host's context panel). On the way back down, `vpnStore.ts::releaseIfUnused(hostId)` is called whenever a session tab closes (`AppShell.tsx::handleCloseTab`) or a tunnel stops (`TunnelsPanel.tsx::handleStop`) - it disconnects that host's VPN profile only if no *other* open session or active tunnel still references it, so one shared profile stays up as long as anything needs it and comes down automatically once nothing does. `vpn::disconnect_all` (wired to Tauri's `RunEvent::ExitRequested` in `lib.rs`, plus a manual "Disconnect all" button in `VpnPanel.tsx`) is the safety net for anything left connected when the app closes or gets stuck - a forgotten/orphaned tunnel silently rerouting traffic (including breaking the user's own internet, if the profile pushes a default-route redirect) was an actual bug hit in testing before this existed.
